...
class _MyAppState extends State<InitialScreen> {
  ...
  Widget build(BuildContext context) {
    if (_customerInfo == null) {
      ...
      final isPro = _customerInfo!.entitlements.active.containsKey(entitlementKey);
      if (isPro) {
      ...
      if (offering != null) {
        List<Widget> buttonThings = offering.availablePackages
            .map((package) {
              ...
              List<Widget> optionButtons = (package.storeProduct.subscriptionOptions?.map((e) {
...
class _PurchaseStoreProductButton extends StatelessWidget {
  ...
  // ignore: public_member_api_docs
  const _PurchaseStoreProductButton({Key? key, required this.storeProduct}) : super(key: key);
...
class _PurchaseSubscriptionOptionButton extends StatelessWidget {
  ...
  // ignore: public_member_api_docs
  const _PurchaseSubscriptionOptionButton({Key? key, required this.option}) : super(key: key);
  ...
  Widget build(BuildContext context) => ElevatedButton(
        ...
        child: Text('Buy Option: - (${option.id}\n${option.pricingPhases.map((e) {
            ...
            ElevatedButton(
              onPressed: () async {
                try {
                  final customerInfo = await Purchases.getCustomerInfo();
                  final refundStatus =
                      await Purchases.beginRefundRequestForEntitlement(customerInfo.entitlements.active[entitlementKey]!);
